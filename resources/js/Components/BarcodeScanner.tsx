...
interface Props {
     ...
     *
     * True in the search dialog: someone who has just pressed a scan button has
     * already expressed the intent, and making them press a second one inside
     * the dialog is a step that exists only because of how the code is
     * organised. False on the standalone page, where the button is the page.
     */
    autoStart?: boolean
...
export default function BarcodeScanner({ autoStart = false, onFound }: Props) {
    ...
    // Open the camera on mount when asked. The dialog uses this; the page does
    // not, because there the button IS the page.
    useEffect(() => {
        if (autoStart) void start()
    }, [autoStart, start])

    // Tell the host once a lookup succeeds, so a dialog can close itself
    // instead of leaving the camera running behind a result.
    useEffect(() => {
