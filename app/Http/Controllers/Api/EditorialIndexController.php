    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'endpoints' => [
                ...
                'GET  /api/editorial/interests/candidates?market=&days=&limit=' => 'The interests people typed into the gift wizard that the vocabulary does not have, ranked by how often. Adding one is a code change (a case on Interest, a seed in AngleMap, four labels); this says which are worth it.',
                'GET  /api/editorial/products/untagged?market=&limit=&after=' => 'Products on an editorial surface with no gift tags yet, plus the vocabulary to tag them from: interest:<interest>, occasion:<occasion>, recipient:<type>. Page with after=<last id>.',
                'POST /api/editorial/products/tags' => 'Write gift tags in batch: {market, tags: [{id, tags: ["interest:coffee", "recipient:mother"]}]}, up to 200, all or nothing, replacing what each product had; an empty set clears. A tag outside the vocabulary refuses the batch. Publish ability: the Whisperer reads them at once.',
                ...
                'POST /api/editorial/coves' => 'Write or rewrite one plan whole — kind, address, shortlist and all. Creates a draft. It REPLACES the shortlist, so prefer the narrower calls below once a plan exists.',
                'PATCH /api/editorial/coves/{id}' => 'Change the settings on a plan — pickMode, writer, buildInstructions, queries, focusKeyphrase — without touching the shortlist or the prose.',
                'POST /api/editorial/coves/{id}/items' => 'Add one product to the shortlist, by ean (portable across markets and hosts, so prefer it), groupId, or source+externalId.',
                'PATCH /api/editorial/coves/{id}/items' => 'Reorder the shortlist and write the note, verdict and card copy for each item, in one call.',
                'DELETE /api/editorial/coves/{id}/items/{itemId}' => 'Take one product off the shortlist.',
                'POST /api/editorial/coves/{id}/suggest' => 'Top the shortlist up with what the builder would have chosen. Only ever adds; existing items keep their position and their notes.',
                'GET  /api/editorial/coves/{id}/conflicts' => 'Where else these products are already spoken for. Advisory, never a filter.',
